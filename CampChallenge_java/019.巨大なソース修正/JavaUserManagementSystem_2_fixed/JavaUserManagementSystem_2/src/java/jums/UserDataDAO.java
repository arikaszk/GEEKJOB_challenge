...
public class UserDataDAO {
    ...
    public ArrayList<UserDataDTO> search(UserDataDTO ud) throws SQLException{
        ...
        try{
            ...
            boolean flag = false;
            int check = 0;  //二進法でどの検索方法を追加しているかを判断する
    ...
    public void update(UserDataDTO ud) throws SQLException {
        ...
        try{
            ...
            String sql = "INSERT INTO user_t(userID,name,birthday,tell,type,comment,newDate) VALUES(?,?,?,?,?,?,?)"
                    + "ON DUPLICATE KEY UPDATE name=VALUES(name),birthday=VALUES(birthday),tell=VALUES(tell),type=VALUES(type),comment=VALUES(comment),newDate=VALUES(newDate)";
            st = con.prepareStatement(sql);
            st.setInt(1, ud.getUserID());
